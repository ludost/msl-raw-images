...
public class CollectorAgent extends Agent {
	...
	public ArrayNode _getSol(int sol,boolean camera) {
		...
		URL url;
		try {
			url = new URL(BASEURL + "?"+(new Random().nextInt())+(camera?"&camera=":"")+"&s=" + sol);
			...
			con.setRequestProperty("User-Agent","Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.11 (KHTML, like Gecko) Chrome/23.0.1271.97 Safari/537.11");
	...
	public void endElement(String uri, String localName, String qName)
		...
		if (doDate && "div".equals(qName)) {
			...
			if (current != null && dateString.length() > 0) {
				...
				try {
					DateTime date = fmt.parseDateTime(dateString.substring(0, 19));
					if (date != null) {
